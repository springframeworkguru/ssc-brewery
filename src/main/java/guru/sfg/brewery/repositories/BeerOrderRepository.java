...
public interface BeerOrderRepository extends JpaRepository<BeerOrder, UUID> {
    ...
    @NotNull Page<BeerOrder> findAllByCustomer(@NotNull Customer customer, @NotNull Pageable pageable);

    @Query("select o from BeerOrder o where o.id =?1 and " +
            "(true = :#{hasAuthority('order.read')} or o.customer.id = ?#{principal?.customer?.id})")
    @Nullable BeerOrder findOrderByIdSecure(@NotNull UUID orderId);
